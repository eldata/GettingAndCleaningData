...
5) LOAD TEST AND TRAINING SETS AND THE ACTIVITIES (the work that has been performed to clean up the data):


i) The data set has been stored in the UCI HAR Dataset/ directory.

ii) The CDN url provided by the instructor is used instead of the original location.

iii) The unzip function is used to extract the zip file in this directory.

fileUrl <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
download.file(fileUrl, destfile = "Dataset.zip", method = "auto")  #you might need to change method="curl" if #using Mac OS to execute the script#
unzip("Dataset.zip")


iv) Load the data into the R environment:
